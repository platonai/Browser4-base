...
[English](README.md) | 简体中�?| [中国镜像](https://gitee.com/platonai_galaxyeye/Browser4)
...
💖 **Browser4：为 AI 打造的闪电般快速、协程安�?(coroutine-safe) 的浏览器引擎** 💖

### �?核心能力

* 👽 **浏览器智能体 (Browser Agents)** �?能在浏览器中进行推理、规划并执行操作的自主智能体�?* 🤖 **浏览器自动化** �?面向工作流、导航和数据提取的高性能自动化�?* ⚙️ **机器学习智能�?* �?在复杂页面上学习字段结构，无需消�?Token�?* �? **极致性能** �?完全协程安全；支持单机每天访�?100k ~ 200k 页面�?* 🧬 **数据抽取** �?结合 LLM、ML 与选择器，在复杂页面中获取干净数据�?
## �?快速示例：Agentic 工作�?
```kotlin
// 让你的智能体执行使命，而不是写脚本
val agent = AgenticContexts.getOrCreateAgent()

// 智能体规划、导航、执行，Browser4 充当“手”和“眼�?val result = agent.run("""
    1. Go to amazon.com
...
2. **配置你的 LLM API 密钥**
   ...
   编辑 [application.properties](application.properties) 并添加你�?API Key�?
3. **构建项目（Linux/macOS�?*
   ```shell
   ./mvnw -DskipTests
   ```
...
4. **运行示例（Linux/macOS�?*
   ...
   **Windows (cmd)**�?   ```shell
   ...
   �?`browser4-examples` 模块中探索并运行示例，直观了�?Browser4 的能力�?
Docker 部署请参见我们的 [Docker Hub 仓库](https://hub.docker.com/r/galaxyeye88/browser4)�?
**Windows 用户**：您也可以将 Browser4 构建为独立的 Windows 安装程序，详见 [Windows 安装程序指南](browser4/browser4-agents/README.md)�?
---
...
**特性：**
- 直接且完整的 Chrome DevTools Protocol (CDP) 控制，协程安�?- 精确的元素交互（点击、滚动、输入）
- 基于 CSS 选择�?/ XPath 的快速数据提�?
```kotlin
val session = AgenticContexts.getOrCreateSession()
val agent = session.companionAgent
val driver = session.getOrCreateBoundDriver()

// 打开并解析页�?var page = session.open(url)
var document = session.parse(page)
var fields = session.extract(document, mapOf("title" to "#title"))

// 与页面交�?var result = agent.act("scroll to the comment section")
...
// 捕获并基于当前状态提�?page = session.capture(driver)
...
**优势�?*
- 相比传统方法，可多提�?10 倍实体与 100 倍字�?- 结合 LLM 智能与精�?CSS 选择�?/ XPath
- �?SQL 语法，上手友�?
```kotlin
...
示例代码�?* [使用 X-SQL 从亚马逊商品页抓取 100+ 字段](https://github.com/platonai/exotic-amazon/tree/main/src/main/resources/sites/amazon/crawl/parse/sql/crawl)
...
基于�?/ 无监督机器学习的自动化、大规模、高精度字段发现与抽取——无需 LLM API 调用、无 Token 成本、确定且快速�?
**它能做什么：**
...
**集成状态：**
- 现已通过配套项目 [PulsarRPAPro](https://github.com/platonai/PulsarRPAPro) 提供�?- 计划提供 Browser4 原生 API；关注后续版本发布�?
**关键优势�?*
- 高精度：>95% 字段被发现；多数字段精度 >99%（指示性测试数据）�?- 抗选择器震荡与 HTML 噪声�?- 零外部依赖（无需 API Key），规模化成本更优�?- 可解释：生成的选择器与 SQL 透明可审计�?
👽 利用机器学习智能体抽取数据：
...
| `pulsar-client` | 客户�?SDK / CLI 工具 |
| `browser4-spa` | 面向浏览器智能体的单页应�?|
| `browser4-agents` | 智能体与爬虫编排及产品打�?|
| `pulsar-tests` | 重型集成与场景测�?|
| `pulsar-tests-common` | 共享测试工具与夹�?|
...
将环境变�?PROXY_ROTATION_URL 设置为代理服务提供的轮换 URL�?
```shell
export PROXY_ROTATION_URL=https://your-proxy-provider.com/rotation-endpoint
```

每次访问该轮�?URL，应返回包含一个或多个新鲜代理 IP 的响应�?如需该类 URL，请联系你的代理服务商�?
</details>
...
## �?功能特�?
### AI 与智能体
- 面向问题求解的自主浏览器智能�?- 并行智能体会�?- LLM 辅助的页面理解与抽取

### 浏览器自动化�?RPA
- 基于工作流的浏览器动�?- 协程安全的精确控制（滚动、点击、抽取）
- 灵活的事件处理与生命周期管理

### 数据抽取与查�?- 一行命令完成数据抽�?- 面向 DOM/内容�?X-SQL 扩展查询语言
- 结构�?+ 非结构化的混合抽取（LLM + 选择器）

### 性能与可扩展�?- 高效并行页面渲染
- 抗封锁设计与智能重试
- 在普通硬件上达到 100,000+ �?天（指示性）

### 隐匿与可靠�?- 先进的反机器人技�?- IP 与配置文件轮�?- 弹性调度与质量保证

### 开发者体�?- 简单的 API 集成（REST、原生、文本命令）
- 丰富的配置分�?- 清晰的结构化日志 (Structured Logging) 与指�?
### 存储与监�?- 本地文件系统�?MongoDB 支持（可扩展�?- 全面日志与透明�?- 细致的指标与生命周期可观测�?
---
...
> 英文文档请参�?[English README](README.md)�?
---

**术语速览（维护一致性）�?*
- Browser Agents：浏览器智能体（首次出现保留英文�?- AgenticSession / AgenticContexts：保持英�?- WebDriver：保持英�?- Chrome DevTools Protocol (CDP)：Chrome DevTools 协议 (CDP)
- X-SQL：保持英�?- LLM：大语言模型 (LLM)
